// SignUp.tsx - User registration form for new users
...
export default function SignUp() {
  ...
  return (
    <form
      ...
      autoComplete="off"
    >
      <h2 className="text-2xl sm:text-3xl font-extrabold mb-2 text-center text-gray-900 dark:text-white tracking-tight">Sign Up</h2>
      <p className="text-gray-500 dark:text-gray-300 text-center mb-4">Create your account to get started.</p>
      {error && (
      ...
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          required
          className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 transition bg-white text-gray-900 border-gray-300 focus:ring-blue-500 placeholder-gray-400 dark:bg-gray-900 dark:text-white dark:border-gray-700 dark:focus:ring-blue-400 dark:placeholder-gray-400 ${confirmTouched && !passwordsMatch ? 'border-red-500 focus:ring-red-400 dark:border-red-500 dark:focus:ring-red-400' : ''}`}
          aria-invalid={confirmTouched && !passwordsMatch}
          aria-describedby="confirm-error"
        />
        {confirmTouched && !passwordsMatch && (
          <span id="confirm-error" className="text-xs text-red-500 mt-1">Passwords do not match.</span>
        )}
      </div>
      <button
        type="submit"
        ...
        disabled={loading || !isEmailValid(email) || !password || !confirmPassword || !passwordsMatch}
      >
        {loading && (
